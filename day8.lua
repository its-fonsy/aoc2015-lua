...
local function code_char(str)
	return #str
end

local function mem_char(str)
	local s = str:sub(2, #str - 1)
	local c = ''
	local i = 1
	local mem_char = 0
	...
	while i <= #s do
		...
		-- if char is '\' then escape next char(s)
		if c == '\\' then
			c = s:sub(i+1, i+1)
			if c == '"' or c == '\\' then
				i = i + 2
...
local function encode_string(str)
	...
	while i <= #str do
		c = str:sub(i,i)
		if c == "\\" or c == "\"" then
			encode_chars = encode_chars + 2
		else
			encode_chars = encode_chars + 1
		end
		i = i + 1
	end
...
for line in input:gmatch("[^\n]+") do
	m = m + mem_char(line)
	c = c + code_char(line)
	e = e + encode_string(line)
end

print(string.format("total [m]emory characters %d", m))
print(string.format("total [c]ode characters %d", c))
print(string.format("total [e]ncoded characters %d", e))
print(string.format("c - m = %d - %d = %d", c, m, c - m))
print(string.format("e - c = %d - %d = %d", e, c, e - c))
