	...
	for j = x, w do
		for k = y, h do
			...
			elseif line:find("toggle") then
				...
				bgrid[j][k] = bgrid[j][k] + 2
			end
		end
	end
end

-- count the light that stayied light up
local n = 0
local brightness = 0
for j = 0, 999 do
	for k = 0, 999 do
		...
		brightness = brightness + bgrid[j][k]
	end
end

print("The lights that stayed on are " .. n)
